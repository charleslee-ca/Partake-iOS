//
//  CLCollectionViewReorderableMasterDetailLayout.m
//  Partake
//
...
typedef NS_ENUM(NSInteger, CLScrollDirection) {
    ...
    CLScrollDirectionUp,
    CLScrollDirectionDown
};


@interface UIImageView (CLCollectionViewReorderableMasterDetailLayout)

- (void)setCellCopiedImage:(UICollectionViewCell *)cell;

@end

@implementation UIImageView (CLCollectionViewReorderableMasterDetailLayout)

- (void)setCellCopiedImage:(UICollectionViewCell *)cell {
    if ([cell isKindOfClass:[CLProfileImageCell class]]) {
        CLProfileImageCell *imageCell = (CLProfileImageCell *)cell;
        self.image = imageCell.imageView.image;
...
@interface CLCollectionViewReorderableMasterDetailLayout()

@property (nonatomic, strong) UIView *cellFakeView;
@property (nonatomic, strong) CADisplayLink *displayLink;
@property (nonatomic, assign) CLScrollDirection scrollDirection;
...
@implementation CLCollectionViewReorderableMasterDetailLayout
@dynamic scrollDirection;

#pragma mark - Override methods

- (void)setDelegate:(id<CLCollectionViewDelegateReorderableMasterDetailLayout>)delegate
{
    self.collectionView.delegate = delegate;
}

- (id<CLCollectionViewDelegateReorderableMasterDetailLayout>)delegate
{
    return (id<CLCollectionViewDelegateReorderableMasterDetailLayout>)self.collectionView.delegate;
}

- (void)setDatasource:(id<CLCollectionViewReorderableMasterDetailLayoutDataSource>)datasource
{
    self.collectionView.dataSource = datasource;
}

- (id<CLCollectionViewReorderableMasterDetailLayoutDataSource>)datasource
{
    return (id<CLCollectionViewReorderableMasterDetailLayoutDataSource>)self.collectionView.dataSource;
}
...
- (void)autoScroll
{
    ...
    if (self.scrollDirection == CLScrollDirectionDown) {
    ...
    }else if (self.scrollDirection == CLScrollDirectionUp) {
...
- (void)handleLongPressGesture:(UILongPressGestureRecognizer *)longPress
{
    switch (longPress.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            //animation
            CGSize cellSize = CGSizeMake(100.f, 100.f);
            if ([self.delegate respondsToSelector:@selector(collectionView:layout:sizeForItemAtIndexPath:)]) {
                cellSize = [self.delegate collectionView:self.collectionView layout:self sizeForItemAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:0]];
            }
            
            CGRect fakeViewRect = CGRectMake(cell.center.x - (cellSize.width / 2.f), cell.center.y - (cellSize.height / 2.f), cellSize.width, cellSize.height);
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            if (CGRectGetMaxY(_cellFakeView.frame) >= self.collectionView.contentOffset.y + (self.collectionView.bounds.size.height - _scrollTrigerEdgeInsets.bottom -_scrollTrigePadding.bottom)) {
                if (ceilf(self.collectionView.contentOffset.y) < self.collectionView.contentSize.height - self.collectionView.bounds.size.height) {
                    self.scrollDirection = CLScrollDirectionDown;
                ...
                if (self.collectionView.contentOffset.y > -self.collectionView.contentInset.top) {
                    self.scrollDirection = CLScrollDirectionUp;
                    [self setUpDisplayLink];
                }
            }else {
